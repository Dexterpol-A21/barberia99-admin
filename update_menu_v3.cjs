...
<DashboardLayout title="Menú de Servicios | Barbería 99">
                                                ...
                                                <tr class="hover:bg-white/5 transition-colors group">
                                                    <td class="p-3 pl-4">
                                                        <div class="flex items-start gap-2">
                                                            <button 
                                                                class={\`p-1 mt-0.5 rounded-full transition-colors cursor-pointer btn-toggle-featured \${servicio.is_featured ? 'text-amber hover:text-cement' : 'text-cement/30 hover:text-amber/50'}\`} 
                                                                data-id={servicio.id} 
                                                            ...
                                                            <div class="flex flex-col">
                                                                <span class={\`font-medium \${servicio.is_active ? 'text-bone' : 'text-cement/50 line-through'} truncate max-w-[150px] sm:max-w-[200px]\`} title={servicio.name}>
                                                                    {servicio.name}
                                                                </span>
                                                                {servicio.description && (
                                                                    <span class="text-[10px] text-cement/70 line-clamp-1 max-w-[180px] sm:max-w-xs mt-0.5" title={servicio.description}>
                                                                        {servicio.description}
                                                                    </span>
                                                                )}
                                                    ...
                                                    <td class="p-3 text-right hidden sm:table-cell align-top">
                                                        <div class="text-cement mt-0.5">{servicio.duration_minutes} min</div>
                                                    </td>
                                                    <td class="p-3 text-right align-top">
                                                        {pricing.isDiscounted ? (
                                                            ...
                                                            <div class="text-amber font-body font-bold mt-0.5">\${pricing.finalPrice}</div>
                                                        )}
                                                    </td>
                                                    <td class="p-3 pr-4 text-right align-top">
                                                        <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
    ...
    <dialog id="modal-form-service" class="bg-transparent m-auto p-0 backdrop:bg-black/80 backdrop:backdrop-blur-sm shadow-2xl rounded-xl open:flex flex-col fixed inset-0 z-50">
        <div class="bg-[#0a0806] border border-white/10 w-[90vw] max-w-md mx-auto p-6 flex flex-col gap-4">
            ...
            <form id="form-service" class="flex flex-col gap-3">
                ...
                <div>
                    <div class="flex justify-between items-end mb-1">
                        <label class="block text-xs text-cement">Nombre del Servicio</label>
                        <span class="text-[10px] text-cement/50" id="char-name">0/45</span>
                    </div>
                    <input type="text" id="input-name" maxlength="45" class="w-full bg-[#15120f] border border-white/10 rounded-sm px-3 py-2 text-bone focus:border-amber outline-none" placeholder="Ej. Corte Clásico" required />
                </div>

                <div>
                    <div class="flex justify-between items-end mb-1">
                        <label class="block text-xs text-cement">Descripción Corta (App Cliente)</label>
                        <span class="text-[10px] text-cement/50" id="char-desc">0/160</span>
                    </div>
                    <textarea id="input-desc" maxlength="160" rows="2" class="w-full bg-[#15120f] border border-white/10 rounded-sm px-3 py-2 text-sm text-bone focus:border-amber outline-none resize-none" placeholder="Breve y persuasiva..." required></textarea>
                </div>
                ...
                <div class="bg-white/5 p-3 rounded border border-white/5 mt-1">
...
<script>
    ...
    // Char counters updates
    inputName.addEventListener('input', () => charName.innerText = \`\${inputName.value.length}/45\`);
    inputDesc.addEventListener('input', () => charDesc.innerText = \`\${inputDesc.value.length}/160\`);
    ...
    function openModal(service = null, defaultCategory = "cortes") {
        ...
        charName.innerText = \`\${inputName.value.length}/45\`;
        charDesc.innerText = \`\${inputDesc.value.length}/160\`;
